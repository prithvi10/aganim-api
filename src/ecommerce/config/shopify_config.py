"""
Shopify / e-commerce specific configuration.

These settings are domain-specific and do NOT belong in the generic
``agentic_core`` layer.

This module also re-exports everything from ``src.shared.config.configs``
so that callers can import *all* config constants from a single location:
``from src.ecommerce.config.shopify_config import X``.
"""
from __future__ import annotations

import os

# Re-export shared (generic) configuration so callers have one import target
from src.shared.config.configs import *  # noqa: F401,F403

# ------------------------------------------------------------------------------
# Model tiers (Pro / Degraded) — used by Fair-Use monitoring
# ------------------------------------------------------------------------------
# Pro-only model (may be throttled down by Fair Use Monitoring)
OPENAI_MODEL_PRO = os.getenv("OPENAI_MODEL_PRO", "gpt-4o-mini")

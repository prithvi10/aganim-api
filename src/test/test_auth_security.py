...
from src.shared.security.security import verify_shopify_redirect, verify_shopify_proxy_request
...
def test_verify_shopify_redirect_success(valid_params):
    ...
    with patch("src.shared.security.security.SHOPIFY_API_SECRET", MOCK_SECRET):
...
def test_verify_shopify_redirect_missing_config():
    ...
    with patch("src.shared.security.security.SHOPIFY_API_SECRET", None):
        with pytest.raises(HTTPException) as exc_info:

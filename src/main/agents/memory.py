...
class AgentMemoryService:
    ...
    async def record_success(
        ...
        This is lighter-weight than corrections - just for tracking
        what worked well.
        ...
        """
        # For now, just log it. Future: Could store in a separate table
    ...
    async def record_failure(
        ...
        Args:
            agent_role: The agent role
            tool_name: The tool/service that failed
            error: Error message
        """
        logger.warning(
    ...
    async def get_similar_corrections(
        ...
        This enables semantic matching - if the user corrected a similar
        product before, we can apply that learning.
        
        Args:
            agent_role: The agent role
            query_text: Text to match against (product title/description)
            limit: Maximum results
        
        Returns:
            List of similar correction dicts
        """
        if not self.db:
            return []
